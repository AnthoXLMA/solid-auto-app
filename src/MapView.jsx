...
export default function MapView({ reports = [], solidaires = [], alerts = [], userPosition, onReportClick, onAlertUser, activeReport, selectedAlert, cancelReport, currentUserUid }) {
  useEffect(() => {
    ...
    const unsub = onSnapshot(reportRef, (docSnap) => {
      if (!docSnap.exists()) cancelReport(activeReport.id);
        ...
        if (data.status !== activeReport.status) onReportClick({ ...activeReport, status: data.status, helperUid: data.helperUid });
      }
    });
    return () => unsub();
  ...
  if (selectedAlert) {
    ...
    if (report && report.latitude && report.longitude) alertLocation = { latitude: report.latitude, longitude: report.longitude };
  }

  if (!userPosition || userPosition.length < 2 || userPosition[0] == null || userPosition[1] == null) return <div>📍 Localisation en cours...</div>;
  ...
  return (
    <MapContainer center={userPosition} zoom={13} style={{ height: "500px", width: "100%", zIndex: 0 }} scrollWheelZoom>
      ...
      {solidaires.map((s) => {
        let status = "relevant";
        const isAlerted = activeReport ? alerts.some((a) => a.reportId === activeReport.id && a.toUid === s.uid) : false;
        if (activeReport?.helperUid === s.uid && activeReport.status === "aide en cours") status = "busy";
        else if (isAlerted) status = "alerted";
        ...
        return (
          <Marker key={s.uid} position={[s.latitude, s.longitude]} icon={getSolidaireIconWithBadge(status)}>
            <Popup>
              ...
              {status === "relevant" && s.uid !== currentUserUid && <button onClick={() => onAlertUser(s)}>⚡ Alerter</button>}
            </Popup>
          </Marker>
        );
      })}
